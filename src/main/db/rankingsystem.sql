...
CREATE TABLE FBJ_TEST (
		...
		,FK_TestNameID INT REFERENCES FBJ_TEST_NAME(PK_TestNameID)
		,Updt DATETIME Default GETDATE()
	);
...
GO
Print 'FBJ_TEST_NAME_ITEM Table.' + CHAR(10);

CREATE TABLE FBJ_TEST_NAME_ITEM
    (
    PK_TestNameItemID int IDENTITY(1,1) NOT NULL PRIMARY KEY,
    FK_TestNameID int NOT NULL REFERENCES FBJ_TEST_NAME(PK_TestNameID),
    FK_ItemID int NOT NULL REFERENCES FBJ_ITEM(PK_ItemID)
    );

GO
Print 'Create FBJ_TEST_NAME Table.' + CHAR(10);

    CREATE TABLE FBJ_TEST_NAME
    (
    PK_TestNameID int NOT NULL IDENTITY(1,1) PRIMARY KEY,
    Name varchar(20) NOT NULL
    );
